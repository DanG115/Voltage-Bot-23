...
PLAYING = [
  'Proteting your server!', 'Helping Servers', 'Music', 'None your buisness.',
  'Moderating', 'Amoung Us', 'Fortnite', 'COD', 'Discord'
]
WATCHING = [
  'Paint dry', 'Music videos', 'Moderation', 'Youtube', 'Reddit', 'The Sky',
  'Your Server'
]
ACTIVITYTYPE = {
...
async def prefix_error(ctx, error):
  if isinstance(error, commands.MissingRequiredArgument):
    await ctx.send(
      'Please list a prefix that you would like to change to. Voltages prefix will be set back to the defult'
    )
...
@client.group(invoke_without_command=True,
              aliases=['modhelp', 'ModerationHelp', 'Moderationhelp'])
async def ModHelp(ctx):
  em = discord.Embed(
    title="Moderation Help",
    description=
    " Use v!modhelp <command> for extended infomation on a command.",
    color=discord.Color.from_rgb(83, 84, 55))
...
async def FunHelp(ctx):
  em = discord.Embed(
    ...
    description=
    " Use v!funhelp <command> for extended infomation on a command.",
    color=discord.Color.from_rgb(204, 131, 104))
...
@client.group(invoke_without_command=True,
              aliases=['statshelp', 'statisticshelp'])
async def StatsHelp(ctx):
  em = discord.Embed(
    title="Stats Commands Help",
    description=
    " Use v!statshelp <command> for extended infomation on a command.",
    color=discord.Color.from_rgb(104, 204, 154))
...
@client.group(invoke_without_command=True,
              aliases=['Setuphelp', 'settingshelp'])
async def SettingsHelp(ctx):
  em = discord.Embed(
    title="Stats Commands Help",
    description=
    " Use v!settingshelp <command> for extended infomation on a command.",
    color=discord.Color.from_rgb(204, 104, 194))
...
async def ban(ctx):
  em = discord.Embed(title="Ban",
                     ...
                     color=discord.Color.from_rgb(0, 81, 255))
...
async def unban(ctx):
  em = discord.Embed(title="Unban",
                     ...
                     color=discord.Color.from_rgb(117, 255, 0))
...
async def slowmode(ctx):
  em = discord.Embed(title="Slowmode",
                     description="Slows down a specific Channel.",
                     color=discord.Color.from_rgb(164, 162, 233))
...
async def cr(ctx):
  embed = discord.Embed(
    title="CoolRate",
    description=f"You are {random.randrange(101)}% Cool {ctx.author.mention}",
...
async def gr(ctx):
  embed = discord.Embed(
    title="Gayrate",
    description=f"You are {random.randrange(101) }% Gay {ctx.author.mention}",
    color=discord.Color.random())
  await ctx.send(embed=embed)
...
async def ban(ctx, user: discord.Member, *, reason="No reason provided"):
  ...
  ban = discord.Embed(
    ...
    description=f"Reason: {reason}\nBy: {ctx.author.mention}",
    color=discord.Color.from_rgb(255, 0, 157))
...
async def unban(ctx, user: discord.User):
  ...
  mbed = discord.Embed(title='Success!',
                       ...
                       color=discord.Color.from_rgb(61, 255, 0))
...
async def slowmode(ctx, seconds: int):
  ...
  mbed = discord.Embed(
    title='Slowmode!',
    description=f"This channels slowmode has been set to {seconds} seconds ⏲️",
    color=discord.Color.from_rgb(188, 255, 0))
  await ctx.send(embed=mbed)
...
async def clear(ctx, amount: int):
  ...
  mbed = discord.Embed(
    ...
    description=f"{amount} Message(s) Cleared by {ctx.author.mention}",
    color=discord.Color.from_rgb(188, 255, 0))
  await ctx.send(embed=mbed)
